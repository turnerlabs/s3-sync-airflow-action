...
if [ -d "$SOURCE_DIR/variables" ]; then
  ...
  if [ "`aws s3 ls s3://$S3_AIRFLOW_BUCKET/variables/`" != "" ];
...
if [ -d "$SOURCE_DIR/requirements" ]; then
  ...
  if [ "`aws s3 ls s3://$S3_AIRFLOW_BUCKET/requirements/`" != "" ];
...
if [ -d "$SOURCE_DIR/dags" ]; then
  ...
  if [ "`aws s3 ls s3://$S3_AIRFLOW_BUCKET/dags/`" != "" ];
...
if [ -d "$SOURCE_DIR/plugins" ]; then
  ...
  if [ "`aws s3 ls s3://$S3_AIRFLOW_BUCKET/plugins/`" != "" ];
...
if [ -d "$SOURCE_DIR/sql" ]; then
  ...
  if [ "`aws s3 ls s3://$S3_AIRFLOW_BUCKET/sql/`" != "" ];
...
if [ -d "$SOURCE_DIR/turner_lib" ]; then
  ...
  if [ "`aws s3 ls s3://$S3_AIRFLOW_BUCKET/turner_lib/`" != "" ];
